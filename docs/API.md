...
`unreadCount` はそのまま採用せず、当てた後に手元で数え直す（`u` で未読に戻した記事はサーバが知らず、保持期間で手元から消した記事はサーバにまだある）。
...
`url` はサイト URL でもフィード URL でもよい。サイト URL の場合は `<link rel="alternate" type="application/rss+xml">` を探してフィード URL を発見する（`HTMLRewriter` を使う）。**`<link>` が 1 つも無ければ、同一サイト内のフィードらしい `<a href>`（`.xml` / `.rdf` / `.atom` / `.rss` で終わる、または `/feed` `/rss` `/atom` で終わる）を候補にする。それでも見つからなければ、パスを 1 段ずつ 3 段まで遡って同じことを試す**（記事詳細ページにフィードの在処を書いていないサイトがある）。**遡って見つけたものは、候補が 1 件でも 300 を返して選ばせる。** 貼られた URL とは違う場所のフィードなので、黙って登録しない。

**RSS を出していないが取り込み方を決め打ちで持っているサイト**（`docs/DESIGN.md` の「RSS を出さないサイトを取り込む」）の URL は、ページを取らずにそのままフィードとして登録する。

登録後、**同期的に初回クロールを実行**してから返す。ユーザが登録直後に記事を読めるようにするため。

候補が複数あるか、上の階層まで遡って見つけた場合は 300 相当を返し、候補を提示する。応答には候補を見つけたページ（`foundAt`）を含める。**貼られた URL と違うことがある**ので、画面はそれを伝えたうえで選ばせる。
